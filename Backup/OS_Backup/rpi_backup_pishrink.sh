...
# --- Pause Docker services (HA, Z2M, Mosquitto) for a consistent backup ---
# Containers we pause temporarily for the backup
DOCKER_CTRS=(homeassistant zigbee2mqtt mosquitto)
declare -a RUNNING_BEFORE=()

pause_services_for_backup() {
  echo "Preparing containers for backup (stopping running ones): ${DOCKER_CTRS[*]}" | tee -a "$LOG_PATH"
  if $DRY_RUN; then return 0; fi
  # Discover currently running containers
  ...
  if ((${#RUNNING_BEFORE[@]})); then
    echo "Stopping: ${RUNNING_BEFORE[*]}" | tee -a "$LOG_PATH"
    docker stop "${RUNNING_BEFORE[@]}" | tee -a "$LOG_PATH" || true
  else
    echo "None of the target containers are running - nothing to stop." | tee -a "$LOG_PATH"
  fi
}
...
resume_services_after_backup() {
  if $DRY_RUN; then return 0; fi
  if ((${#RUNNING_BEFORE[@]})); then
    echo "Starting again: ${RUNNING_BEFORE[*]}" | tee -a "$LOG_PATH"
    docker start "${RUNNING_BEFORE[@]}" | tee -a "$LOG_PATH" || true
  fi
}

# Ensure services are resumed even on failure
...
# After the raw image is created we can start services again
resume_services_after_backup
# Remove the EXIT trap: services are already back online
trap - EXIT
